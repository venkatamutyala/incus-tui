...
- **Edit CPU/RAM/disk** on an existing VM (`limits.cpu` / `limits.memory` and the root
  ...
  be stopped** to resize it — safe across every storage driver. A grow is **confirmed**
  (old → new) before it applies; cloud images grow the guest filesystem onto the bigger
...
launch disk pool from your default profile.
